...
function videoStorageKey() {
  ...
  return `youtubeVideos_${currentUser}`;
}

function getTrackedVideos() {
  return JSON.parse(localStorage.getItem(videoStorageKey())) || [];
}

function saveTrackedVideos(videos) {
  localStorage.setItem(videoStorageKey(), JSON.stringify(videos));
}

function trackYouTubeVideo(event) {
  event.preventDefault();
  ...
  if (!videoId) {
    message.textContent = "Please paste a valid YouTube URL.";
    return;
  }

  const videos = getTrackedVideos();
  const existing = videos.find(video => video.id === videoId);

  if (existing) {
    existing.viewsTracked += 1;
    existing.lastOpened = new Date().toISOString();
    message.textContent = "This video is already tracked. Count updated.";
  } else {
    videos.unshift({
      id: videoId,
      url: `https://www.youtube.com/watch?v=${videoId}`,
      title: titleInput.value.trim() || "Untitled YouTube Video",
      watched: false,
      viewsTracked: 1,
    ...
    message.textContent = "Video added.";
  }

  saveTrackedVideos(videos);
  renderYouTubeTracker(videoId);

  urlInput.value = "";
  titleInput.value = "";
}

function renderYouTubeTracker(activeId = null) {
  ...
  count.textContent = `${videos.length} video${videos.length === 1 ? "" : "s"} tracked`;

  if (activeVideo) {
    player.innerHTML = `
      <iframe
        src="${buildYouTubeEmbedUrl(activeVideo.id)}"
        title="${escapeHTML(activeVideo.title)}"
      ...
      <p class="youtube-help">
        ...
        <a href="https://www.youtube.com/watch?v=${activeVideo.id}" target="_blank">watch it on YouTube</a>.
      </p>
    `;
  } else {
    player.innerHTML = `<p style="color:#aaa;padding:25px;">Paste a YouTube URL above to preview it here.</p>`;
  }
  ...
  list.innerHTML = videos.length
    ? videos.map(video => `
      <article class="review-box youtube-item">
        ...
        <p>Status: ${video.watched ? "Watched" : "Not watched yet"}</p>
        <p>Tracked: ${video.viewsTracked} time${video.viewsTracked === 1 ? "" : "s"}</p>

        <button class="btn" onclick="playTrackedVideo('${video.id}')">Play</button>

        <a class="btn" href="film-detail.html?type=video&youtube=${video.id}">
          View Details
        </a>

        <button class="btn btn-secondary" onclick="toggleWatchedVideo('${video.id}')">
          ${video.watched ? "Mark Unwatched" : "Mark Watched"}
        </button>

        <button class="btn btn-secondary" onclick="deleteTrackedVideo('${video.id}')">
          Delete
        </button>
      </article>
    `).join("")
...
function playTrackedVideo(videoId) {
  const videos = getTrackedVideos();
  const video = videos.find(item => item.id === videoId);

  if (video) {
    video.viewsTracked += 1;
    video.lastOpened = new Date().toISOString();
  }
...
function toggleWatchedVideo(videoId) {
  const videos = getTrackedVideos();
  const video = videos.find(item => item.id === videoId);
...
function deleteTrackedVideo(videoId) {
  const videos = getTrackedVideos().filter(video => video.id !== videoId);
  saveTrackedVideos(videos);
  renderYouTubeTracker();
}
